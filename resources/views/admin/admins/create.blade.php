...
@section('title', env('APP_NAME').' | '.__('Create Administrator'))

@section('content_header')
    <h1>@lang('Create Administrator')</h1>
@stop

@section('content')
    <div class="card card-default">
        <div class="card-header">
            @lang('Create Administrator')
                    ...
                    <div class="col-sm-10">
                        <select class="form-control" id="roles" name="roles[]" multiple>
                            ...
                            @foreach($Roles as $role)
                                <option value="{{ $role->name }}">{{ \Illuminate\Support\Str::studly($role->name) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
